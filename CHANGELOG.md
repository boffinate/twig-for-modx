...
- Add `uncached_snippet(name, properties = {})`: returns the literal `[[!...]]` tag so MODX's uncacheable pass runs the snippet per request instead of baking its output into the cached render (cache-tag headers being the motivating case)
- Chunks whose source contains no Twig syntax now skip the proxy's property merge and render straight through, and the proxy's unreachable class-constant reflection is gone
- Add `Twig::fromServicesOrLogOnce()` so integrations share the "twigparser unavailable" fallback (resolve the service, log once per request on failure) instead of copy-pasting it
- Remove the ContentBlocks patch instructions: both patches (repeater `row_data`, the `ContentBlocks_BeforeParse` hook) ship in the upstream ContentBlocks package
...
- Compiled templates are no longer auto-reloaded outside debug mode, so production stops stat-ing every template file per request. The TwigCacheClear plugin now also listens on `OnCacheUpdate` (not just `OnSiteRefresh`), so `modCacheManager::refresh()` — what deploy scripts call — clears the compiled templates too. Partial refreshes are exempt: the `OnCacheUpdate` that follows every manager resource save carries no `scripts` partition and now leaves the compiled tree alone, instead of forcing a sitewide recompile per editor save
- `twig.debug` now defaults off. A deployment without an explicit setting previously ran with debug (and auto_reload) enabled — paying the per-template stat cost and returning template source to visitors on render errors
...
- Add `tests/SecurityBoundaryTest.php`: executable coverage of what may and may not be compiled — query-string input echoed into a page, editor content merged through `[[*content]]`, TV values, snippet output — each paired with the case showing what the boundary moving would look like
  ...
  **Upgrading from 0.7.0:** register the two pdoTools class settings if you use pdoMenu/pdoResources tpls with Twig in them. Templates keep working with no action. There is no switch to restore the document pass: element-level rendering covers templates, chunks, ContentBlocks field templates and pdoTools tpl chunks. Chunks fetched by extras the subclasses do not cover (Alpacka-based extras, Tagger) and Twig emitted directly in snippet output no longer render — move that Twig into a chunk or template the covered paths reach
